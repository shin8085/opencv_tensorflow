...
for i in range(0,dstHeight):
    for j in range(0,dstWidth):
        iNew=int(i * (height1 * 1.0 / dstHeight))
...
matMove=np.float32([[1,0,100],
                   [0,1,200]])
dstimg4=cv2.warpAffine(img1, matMove, (width, height1))
...
#-------------图片镜像---------------
dstimg6=np.zeros((height1, width * 2, mode), dtype=np.uint8) #水平镜像
...
matZoom=np.float32([[0.5,0,0],   #见图片位移
                  [0,0.5,0]])
dstimg7=cv2.warpAffine(img1, matZoom, (int(width/2),int(height1 / 2)))
#cv2.imshow("dstimg7",dstimg7)
#cv2.waitKey(0)

#---------------矩阵的仿射变换-------------
#src -> dst 3 (左上角，左下角，右上角）
matSrc=np.float32([[0,0], [0, height1 - 1], [width - 1, 0]])
matDst=np.float32([[50,50], [300, height1 - 200], [width - 300, 100]])
#组合
matAffine=cv2.getAffineTransform(matSrc,matDst)
dstimg8=cv2.warpAffine(img1, matAffine, (width, height1))
#cv2.imshow("dstimg8",dstimg8)
#cv2.waitKey(0)

#--------------------------图片旋转-------------------------
matRotate=cv2.getRotationMatrix2D((width * 0.5, height1 * 0.5), 45, 1) #中心点 旋转角度 缩放系数
dstimg9=cv2.warpAffine(img1, matRotate, (width, height1))
